...
public class FlightDelaysDAO {
	...
	public Map<Flight, Double> getArchi(Airline a){
		String sql = "SELECT *, avg(f.ARRIVAL_DELAY) as mediaRitardi from flights as f where f.AIRLINE = ? group by f.ORIGIN_AIRPORT_ID,f.DESTINATION_AIRPORT_ID";
		Map<Flight, Double> result = new HashMap<Flight,Double>();
		...
		try {
			...
			while (rs.next()) {
				...
				double delay = rs.getDouble("mediaRitardi");
				result.put(flight, delay);
			}
	...
	public List<Flight> getVoliPartenza(Airport a) {
		String sql = "SELECT *from flights as f where f.Origin_airport_id = ? order by scheduled_dep_date asc";
